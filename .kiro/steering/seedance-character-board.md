...
- **NEVER read or load image files in this skill.** Image binaries (`.png`,
  `.jpg`, `.jpeg`, `.gif`, `.webp`, `.heic`, `.bmp`, `.tiff`, `.svg`) MUST
  NOT be opened with any tool (Read, file viewer, etc.) — large images hang
  the agent. Treat any image path the user gives as an opaque string. The
  user attaches the actual image directly to GPT-Image 2 / Nano Banana /
  Midjourney when they paste the final prompt this skill produces.
...
**DO NOT read or open the image file.** This skill MUST NEVER load, parse,
inspect, or describe image binary files (`.png`, `.jpg`, `.jpeg`, `.gif`,
`.webp`, `.heic`, `.bmp`, `.tiff`, `.svg`, etc.) under any circumstances —
including via Read, file viewers, or any other tool. Loading large images
hangs the agent and stalls the workflow. Just record the path the user
provided as a string and proceed. The user will attach the image themselves
to GPT-Image 2 / Nano Banana / Midjourney when they paste the final prompt
generated by this skill.

If the user gives a path, simply confirm:

> Принято: `{{path}}` — использую как референс. Картинку открывать не
> буду, ты её приложишь сам когда вставишь финальный промпт в GPT-Image 2.
...
- **Never read image files.** This skill MUST NOT open or load any image
  binary (`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.heic`, `.bmp`,
  `.tiff`, `.svg`) with any tool. Large images hang the agent. Treat all
  image paths as opaque strings; the user attaches the image directly to
  the downstream tool (GPT-Image 2 / Nano Banana / Midjourney).
